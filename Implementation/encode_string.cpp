...
int main()
{
	...
	printf("%d\n",inp_len);
	int a, b, c = 8 ,num;
	int binary[8 * inp_len];
	for(a = 0; a < 8*inp_len; a++){
	        binary[a] = 0;
	    }

	for(a = 0; a < inp_len; a++){
		b = c - 1;
		num = inp_str[a];//num = *(p+i);
		while(num != 0){
			binary[b--] = num % 2;
			num /= 2;
		}
		c += 9; // because the next position of second character is at index of (7+8)=15
	}
	for(a = 1; a < 8*inp_len+1; a++){
		input.push_back(binary[a - 1]);
	}

	ofstream InFile ("raw_data.txt");           //Creating new file for inputs
	for (int i = 0; i < input.size(); ++i)
	{
        InFile<<input[i]<<"\n";                  //Adding input data to file
	}
	InFile<<"\n";
	...
	cout<<"i/p size is:"<<input.size()<<endl;
