...
int main()
{
	...
	cin>>s;
	cout<<"Taking inputs from File\n";
	//cin>>maxsize;
	//cout<<"Enter the input sequence bit by bit.\n";
	//Taking bit stream as input in vector
	ifstream InFile;
    InFile.open("input_binary.txt");       //Creating new file for inputs
	string temp;
	while(!InFile.eof())
    {
        InFile>>temp;
	...
	ofstream MatEncode ("buffer_encode.txt");
	ofstream OutFile ("encoded_data.txt");              //Creating new file for outputs
	for (int i = 0; i < output.size(); ++i)
	{
		OutFile<<output[i];
		MatEncode<<output[i]<<" ";                       //Adding outputs to file
	}
	//OutFile<<"\n";
	OutFile.close();
	MatEncode.close();
